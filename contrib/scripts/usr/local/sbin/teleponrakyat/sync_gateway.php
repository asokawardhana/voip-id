...
foreach ($data1 as $k1 => $v1) {
	mysql_select_db($db_name1,$conn);
	$r = mysql_query("SELECT custom_parameter from logs WHERE id='$k1' AND event_name='$v1' AND flag IS NULL ORDER BY created_at ASC");
	if(mysql_num_rows($r) == 0){
		$data2 = NULL;
	}else{
		while($row = mysql_fetch_array($r)){
	...
	if(!empty($domain1)){
		...
		if($event=='rm'){
			...
			if(!empty($domain2)){
				if(in_array($domain1, $domain2)) {
					//printf('[INFO] ['.date("Y-m-d H:i:s").'] CANNOT ADD '.$domain1." FOUND ON DB\n");
					//mysql_select_db($db_name1,$conn);
					//$retval = mysql_query("UPDATE logs SET flag='1' WHERE id='$id'");
					//if(!$retval){
					//	die('Could not update data: ' . mysql_error()."\n");
					...
					printf('[INFO] ['.date("Y-m-d H:i:s").'] '.$domain1." FOUND ON DB, REMOVE IT\n");
					$cmd = "/usr/sbin/opensipsctl fifo pdt_delete '*' $domain1";
					$commands[] = $cmd;
				}
...
if ($need_reload) {
	...
	$commands[] = $cmd;
	print('[INFO] ['.date("Y-m-d H:i:s").'] Reload pdt' . "\n");
}
