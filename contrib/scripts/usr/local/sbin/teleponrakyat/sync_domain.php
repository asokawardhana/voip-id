...
foreach ($data1 as $k1 => $v1) {
	...
	if(!empty($domain1)){
		...
		if($event=='rm'){
			if(!empty($domain2)){
				if(in_array($domain1, $domain2)){
					...
					print('[INFO] ['.date("Y-m-d H:i:s").'] FAILED REMOVE DOMAIN '.$domain1." DOMAIN NOT FOUND\n");
				}
			}else{
				mysql_select_db($db_name1,$conn);
				$retval = mysql_query("UPDATE logs SET flag='1' WHERE id='$id'");
			...
			if(!empty($domain2)){
				if(in_array($domain1, $domain2)) {
					...
					print('[INFO] ['.date("Y-m-d H:i:s").'] FAILED ADD DOMAIN '.$domain1." DOMAIN EXISTS\n");
				}else{
		...
		if ($need_reload) {
			$cmd1 = "/usr/sbin/opensipsctl fifo pdt_reload";
			$cmd2 = "/usr/sbin/opensipsctl domain reload";
			$commands = array($cmd1,$cmd2);
			print('[INFO] ['.date("Y-m-d H:i:s").'] Reload domain & pdt' . "\n");
		}

		foreach ($commands as $command) {
			print('[INFO] ['.date("Y-m-d H:i:s").'] EXECUTE '.$command."\n");
			shell_exec($command);
			sleep(3);
		}
